...
/**
 * Cameo 2022x implementation of {@link DiagramApi}.
 *
 * <p>Uses the stable {@code com.nomagic.magicdraw.core} and
 * {@code com.nomagic.magicdraw.uml.symbols} APIs that were present
 * prior to the 2024x restructuring.</p>
 */
public class DiagramApi2022x implements DiagramApi {

    private static final Logger LOG = Logger.getLogger(DiagramApi2022x.class.getName());
    ...
    public String createDiagram(String ownerElementId, String diagramType, String name) {
        ...
        // In 2022x the DiagramTypeManager API is used directly
        DiagramPresentationElement dpe =
                Application.getInstance().getProject()
                        .getDiagramTypeManager()
                        .createDiagram(diagramType, (Package) owner, name);
            ...
            if (dpe.getElement().getID().equals(diagramId)) {
                try {
                    com.nomagic.magicdraw.export.image.ImageExporter.export(
                            dpe, com.nomagic.magicdraw.export.image.ImageExporter.PNG,
                            new File(outputPath));
                } catch (Exception e) {
