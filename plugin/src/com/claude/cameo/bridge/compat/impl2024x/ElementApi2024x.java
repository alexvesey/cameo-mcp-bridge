...
/**
 * Cameo 2024x implementation of {@link ElementApi}.
 *
 * <p>Functionally identical to the 2022x variant for core operations;
 * kept separate so 2024x-specific API additions can be introduced here
 * without branching shared code.</p>
 */
public class ElementApi2024x implements ElementApi {

    private static final Logger LOG = Logger.getLogger(ElementApi2024x.class.getName());

    @Override
    public Map<String, Object> getElement(String elementId) {
        Project project = Application.getInstance().getProject();
        Element el = project.getElementByID(elementId);
        if (el == null) throw new IllegalArgumentException("Element not found: " + elementId);
    ...
    public String createElement(String ownerElementId, String metaclass,
        ...
        if (owner == null) throw new IllegalArgumentException("Owner not found: " + ownerElementId);

        Element newEl = project.getModelFactory().createElement(metaclass);
        if (newEl instanceof NamedElement ne) ne.setName(name);
        try {
            ModelElementsManager.getInstance().addElement(newEl, owner);
        } catch (ReadOnlyElementException e) {
            throw new RuntimeException("Read-only owner: " + ownerElementId, e);
        }
        if (properties != null) applyProperties(newEl, properties);
        return newEl.getID();
    }
    ...
    public List<Map<String, Object>> findElements(String metaclass,
        ...
        for (Element el : candidates) {
            if (ownerId != null && !isDescendantOf(el, ownerId)) continue;
            if (nameGlob != null && el instanceof NamedElement ne
                    && !matchesGlob(ne.getName(), nameGlob)) continue;
    ...
    private Map<String, Object> elementToMap(Element el) {
        ...
        map.put("ownerId", el.getOwner() != null ? el.getOwner().getID() : null);
        return map;
    }

    private void applyProperties(Element el, Map<String, Object> properties) {
        properties.forEach((k, v) -> {
            if ("name".equals(k) && el instanceof NamedElement ne) {
                ne.setName(String.valueOf(v));
            } else {
                LOG.fine("applyProperties: unhandled property '" + k + "'");
            }
        });
    }

    private boolean isDescendantOf(Element el, String ownerId) {
